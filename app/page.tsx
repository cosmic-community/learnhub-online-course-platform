...
export default async function HomePage() {
  ...
  // Find the best beginner course for quick start
  const beginnerCourse = courses.find(
    (course) => course.metadata?.difficulty?.value === 'Beginner' || 
                course.metadata?.difficulty?.value === 'beginner'
  )
  
  // Calculate total learning hours
  const totalHours = courses.reduce(
    (acc, course) => acc + (course.metadata?.estimated_hours || 0), 
    0
  )
  
  // Calculate total lessons across all courses
  const totalLessons = courses.reduce(
    (acc, course) => acc + (course.metadata?.lessons?.length || 0),
    0
  )
  ...
  return (
    <div>
      ...
      {beginnerCourse && (
        <QuickStartSuggestion course={beginnerCourse} />
      )}

      {/* Daily Learning Tip */}
      <DailyLearningTip categories={categories} />
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-primary-400 text-sm font-semibold uppercase tracking-wider">Trending</span>
              </div>
