...
export default async function HomePage() {
  ...
  // Pick "Course of the Day" based on current date (rotates daily)
  const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(), 0, 0).getTime()) / (1000 * 60 * 60 * 24))
  const courseOfTheDayIndex = dayOfYear % Math.max(courses.length, 1)
  const courseOfTheDay = courses[courseOfTheDayIndex] || courses[0]
