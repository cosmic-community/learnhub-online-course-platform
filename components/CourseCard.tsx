...
export default function CourseCard({ course }: CourseCardProps) {
  ...
  return (
    <Link href={`/courses/${course.slug}`} className="card group block relative">
      {/* Hover glow effect */}
      <div className="absolute -inset-0.5 bg-gradient-to-r from-primary-500 to-purple-500 rounded-2xl opacity-0 group-hover:opacity-20 blur transition-opacity duration-300" />
      
      <div className="relative bg-navy-900/50 backdrop-blur-sm border border-navy-800 rounded-2xl overflow-hidden">
        {/* Thumbnail */}
        <div className="relative aspect-video overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-navy-950/30">
            <div className="w-16 h-16 rounded-full bg-primary-500/90 flex items-center justify-center transform group-hover:scale-110 transition-transform">
            ...
            <div className="mt-4 pt-4 border-t border-navy-800 flex items-center gap-3">
              {instructors[0].metadata?.photo ? (
                <img
                  ...
                  width={32}
                  height={32}
                  className="w-8 h-8 rounded-full object-cover ring-2 ring-navy-700 group-hover:ring-primary-500 transition-colors"
                />
              ) : (
