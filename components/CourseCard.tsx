...
export default function CourseCard({ course }: CourseCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/courses/${course.slug}`}>
        ...
        <div className="relative aspect-video overflow-hidden">
          ...
          >
            ...
            >
              <QuickQuiz courseName={course.title} />
            </div>
          </div>
        </div>
      ...
      {/* Hover hint */}
      <div className={`absolute bottom-2 right-2 text-xs text-primary-400 transition-opacity duration-300 ${showQuiz ? 'opacity-100' : 'opacity-0'}`}>
        🎯 Try a quiz!
      </div>
    </div>
  )
